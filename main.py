...
def welcome_message_when_added(message):
  ...
  for user in new_members:
      ...
      if user.username == "YOUR_BOT_USERNAME":
          bot.send_message(chat_id, "Thank you for adding me to this group! I'm here to help admins manage assignments.")
...
def send_help(message):
  ...
  if (message.chat.type == "private"):
    bot.reply_to(message, "I'm here to help admins manage assignments. Please add me to a group to get started.")  
...
def create_assignment_reply(message, user_id):
  ...
  try:
    ...
    logging.info(f'inserted assignment with title {assignment_details["title"]}')
    bot.reply_to(message, "Assignment has been created successfully.")
  except Exception as e:
    bot.reply_to(message, "An error occured while setting assignment. Please try again later.")
    logging.error(e)
...
def list_assignments(message):
  ...
  if (len(ASSIGNMENTS_LIST) > 0):
    for assignment_id in ASSIGNMENTS_LIST:
      ...
      keyboard.row(InlineKeyboardButton('View',callback_data=f'VIEW_{assignment_id}'))
      bot.reply_to(message, f"Course Code: {ASSIGNMENTS_LIST[assignment_id]['course_code']}\n Title: {ASSIGNMENTS_LIST[assignment_id]['title'] }\n Deadline: {ASSIGNMENTS_LIST[assignment_id]['deadline']}\n Description: {ASSIGNMENTS_LIST[assignment_id]['description'][:50]}"
...
def view_assignment(call):
  ...
  bot.reply_to(call.message, f"Course Code: {ASSIGNMENTS_LIST[assignment_id]['course_code']}\n Title: {ASSIGNMENTS_LIST[assignment_id]['title'] }\n Deadline: {ASSIGNMENTS_LIST[assignment_id]['deadline']}\n Description: {ASSIGNMENTS_LIST[assignment_id]['description']}")  
