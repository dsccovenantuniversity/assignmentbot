...
def create_assignment(message):
  ...
  if (message.from_user.id not in admin_ids):
    ...
    return
  assignment_details = message.text.replace("/addassignment", "").strip().split(":")
...
def list_assignments(message):
  ...
  try:
    ...
    logging.error(e)
    bot.reply_to(message, "An error occurred while fetching assignments. Please try again later.")
    return
...
def send_assignment_reminders():
  ...
  for assignment in assignments_list:
    ...
    if (days == 0):
      ...
      try:
        bot.send_message(assignments_list[assignment]['chat_id'],
                 "Assignment with %s is due in %d days. Please submit on time. ⌛" % (assignments_list[assignment]['title'], days))
        logging.info(
          "sent reminder for assignment with title %s" % assignments_list[assignment]['title'])
      except Exception as e:
        logging.error(e)
